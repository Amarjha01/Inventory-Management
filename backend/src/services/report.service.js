...
class ReportService {
  ...
  async downloadRequirementReport(query) {
    ...
    if (dateType === "range") {
      if (!fromDate || !toDate) {
        throw new Error("From date and to date are required for date range.");
      }
  ...
  console.log("item" , item);
    ...
    if (district) {
      filteredRequirements = filteredRequirements.filter((requirement) => {
        const requirementDistrict = requirement.kitchen?.district;

        return (
          requirementDistrict?.toString().toLowerCase() ===
    ...
    console.log("items:", items, "selectedItems:", selectedItems);

    if (selectedItems.length > 0) {
      filteredRequirements = filteredRequirements
        .map((requirement) => {
          const matchingItems = requirement.items?.filter((requirementItem) => {
            const inventoryId =
              requirementItem.inventoryId?._id || requirementItem.inventoryId;

            return selectedItems.includes(inventoryId?.toString());
          });
    ...
    return createRequirementWorkbook(filteredRequirements, {
      status,
      district,
      kitchen,
      items: selectedItems,
  ...
  async getRequirementReportOptions(query) {
    ...
    const requirements = await reportRepository.getRequirements({
      createdAt: {
        $gte: start,
        $lte: end,
      },
    ...
    requirements.forEach((requirement) => {
      ...
      if (requirement.kitchen?._id) {
        kitchensMap.set(requirement.kitchen._id.toString(), {
      ...
      if (requirement.kitchen?.district) {
        const district = requirement.kitchen.district;

        districtsMap.set(district.toString(), {
          _id: district,
          name: district,
        });
      }
      ...
      requirement.items?.forEach((requirementItem) => {
        if (requirementItem.inventoryId?._id) {
          itemsMap.set(requirementItem.inventoryId._id.toString(), {
            _id: requirementItem.inventoryId._id,
            name: requirementItem.inventoryId.name,
          });
        }
      });
    });
