...
const Received = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setRequirements(storage?.getReceivedRequirement || []);
        if (requirements) {
          setLoading(false)
        }
        const data = await getAllKitchenRequirements("Received");
        if (data) {
          storage.setReceivedRequirement(data);
          setRequirements(data || []);
          const stat = storage.getStats();
          storage.setStats({ ...stat, Received: data.length });
        }
      } catch (error) {
        console.error(error);
        if (!requirements) {
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={`Search Received requirement...`}
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">Showing: Received</p>
              </div>
      ...
      {!loading && filteredRequirements?.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed bg-gray-50 py-14 text-center mt-4">
          ...
          <h3 className="mt-3 font-semibold text-gray-700">
            No Received Requirements
          </h3>
          <p className="text-sm text-gray-500">
            {search
              ? "No requirements match your search."
              : `No Received requirements available.`}
          </p>
        </div>
      )}
