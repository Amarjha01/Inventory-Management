...
const Submitted = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setRequirements(storage?.getSubmittedRequirement || []);
        if (requirements) {
          setLoading(false)
        }
        const data = await getAllKitchenRequirements("Submitted");
        if (data) {
          storage.setSubmittedRequirement(data);
          setRequirements(data || []);
          const stat = storage.getStats();
          storage.setStats({ ...stat, Submitted: data.length });
        }
      } catch (error) {
        console.error(error);
        if (!requirements) {
      ...
      <div className="flex gap-2">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={`Search Submitted requirement...`}
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">Showing: Submitted</p>
              </div>
        ...
        <div className="rounded-2xl border-2 border-dashed bg-gray-50 py-14 text-center mt-4">
          ...
          <p className="text-sm text-gray-500">
            {search
              ...
              : `No Submitted requirements available.`}
          </p>
        </div>
      )}
